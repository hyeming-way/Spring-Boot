...
public class StudentController {
	...
	*/
	...
	public String list(Model model,  Pagination pagination) {
		...
		return "student/list";  //list.html로
	...
	public String edit(Model model, @RequestParam("id") int id) {
		 ...
		 model.addAttribute("studentEdit", studentEdit); // DTO가 아니고 모델역할을 하는 StudentEdit객체 저장
		 model.addAttribute("departments", departmentMapper.findAll() );

		 return "student/edit"; //templates/student/edit.html  뷰 요청
...
//	수정 요청 :  POST   http://localhost:8088/student/edit?id=11
	...
	public String edit(Model model,
		...
		try {
			...
			if(bindingResult.hasErrors()) {
				throw new Exception("저장(수정)할수 없습니다.");
			}

			//---  StudentEdit객체의 변수값들을 모두 Student객체에 저장 시키고
			Student student = modelMapper.map(studentEdit, Student.class);

			//	   Student객체의 변수의 정보들을 DB에 저장(수정할 정보 UPDATE)
			studentMapper.update(student);

			return "redirect:list";//웹브라우저 주소창에 http://localhost:8088/student/list 자동입력후
								   //포워딩(조회 재요청)이 이루어진다.
    ...
    public String create(Model model,
    	...
    	try {
    		log.debug("edit.html에서 등록할 학생정보가 저장된 StudentEdit객체의 변수정보들");
    		log.debug(": " + studentEdit.toString());

    		//유효성 검사 통과 하지 않으면?
    		if(bindingResult.hasErrors()) {
    			throw new Exception("학생을 등록할수 없습니다.");
    		}

    		//유효성 검사 통과 하면?  학생등록을 하기 위해
    		//1. StudentEdit객체의 변수정보들을 -> Student객체의 변수들에 전달해서 저장
    		//   (StudentEdit객체를 Student객체로 변환)
    		//방법 : ModelMapper객체 이용
    		Student student = modelMapper.map(studentEdit, Student.class);

    		//2. 학생등록(INSERT)작업하기 위해 StudentMapper인터페이스를 구현한 자식익명객체의
    		//   insert메소드를 호출할때 매개변수로 DTO역할을 하는 바로 위 Student객체 전달하여 DB작업함
			...
			//유효성 검사 통과 하지 않으면? StudentEdit객체를 ""를 작성하여 BindingResult객체에 저장하고
			// e.getMessage() 를 이용해 "학생을 등록할수 없습니다" 예외 메세지를 저장 한후
    ...
    public String delete(Model model,
    	...
    	try {
    		//학생 정보를 삭제 하기 위해 매퍼의 메소드 호출시~~ 매개변수로 삭제할 학생 ID전달
